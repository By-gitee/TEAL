"""
ARM SVE CSR（gather 构建 activation）稀疏 GEMM 算子的正确性与性能测试。

测试输出与测试方式对齐 `scripts/test_sve_csc_gemm.py`。

运行方式:
    python -m scripts.test_sve_sparse_act_csr_gemm
"""
...
from kernels.sve_sparse_gemm import (
    SVESparseActCSRGEMMKernel,
    load_sve_sparse_gemm_extension,
    measure_latency,
    sve_sparse_act_csr_gemm,
    row_scan_sve,
)
...
def _apply_threshold(activation: torch.Tensor, threshold: float = 0.0) -> torch.Tensor:
    """
    对 activation 矩阵应用阈值，绝对值大于 threshold 的值保留，其余置零。
    ...
    """
    return torch.where(torch.abs(activation) > threshold, activation, torch.zeros_like(activation))
...
def check_correctness(seed: int, threshold: float = 0.0) -> None:
    ...
    kernel = SVESparseActCSRGEMMKernel.initialize(name="sve_sparse_act_csr_gemm", target="CPU")
    ...
    # 获取稀疏索引（基于 threshold，使用 row_scan_sve）
    _, nz_col_indices, row_offsets = row_scan_sve(activation, threshold, verbose=False)
...
def test_direct_torch_ops(threshold: float = 0.0) -> None:
    ...
    result_direct = torch.ops.teal.sve_sparse_act_csr_gemm(
...
def main() -> None:
    ...
    parser.add_argument("--threshold", type=float, default=0.8, help="阈值，绝对值大于此值的元素将被保留")
