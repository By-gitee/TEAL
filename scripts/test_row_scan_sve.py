"""
ARM SVE 行扫描算子的正确性与性能测试。

运行方式:
    python -m scripts.test_row_scan_sve
"""
...
def check_correctness(M: int, K: int, threshold: float, seed: int) -> None:
    ...
    # SVE 实现
    nz_counts_sve, nz_col_indices_sve, row_offsets_sve = row_scan_sve(act, threshold, verbose=False)
    ...
    assert int(row_offsets_sve[M].item()) == nz_col_indices_sve.numel(), \
        f"row_offsets[M] 应为 {nz_col_indices_sve.numel()}，实际为 {int(row_offsets_sve[M].item())}"

    # per-row slice check (与 reference 比较)
    for m in range(M):
        s = int(row_offsets_sve[m].item())
        e = int(row_offsets_sve[m + 1].item())
        got = nz_col_indices_sve[s:e].tolist()
        exp = ref_indices[m]
        if got != exp:
    ...
    assert nz_counts_sve.numel() % 2 == 0, "nz_counts 长度应为偶数"
    pairs_sve = nz_counts_sve.view(-1, 2).tolist()
    last_row = -1
    for row, nnz in pairs_sve:
    ...
    # 与 baseline 比较（统一类型：SVE 返回 uint32，baseline 返回 int64）
    nz_col_indices_sve_int64 = nz_col_indices_sve.to(dtype=torch.int64)
    assert torch.equal(nz_counts_sve, nz_counts_baseline), \
        f"SVE 与 baseline 的 nz_counts 不匹配"
    assert torch.equal(nz_col_indices_sve_int64, nz_col_indices_baseline), \
        f"SVE 与 baseline 的 nz_col_indices 不匹配"
    assert torch.equal(row_offsets_sve, row_offsets_baseline), \
        f"SVE 与 baseline 的 row_offsets 不匹配"

    print(f"✅ 正确性测试通过")
    print(f"   总非零元素数: {nz_col_indices_sve.numel()}/{M*K} ({100*nz_col_indices_sve.numel()/(M*K):.1f}%)")
    print(f"   非零行数: {len(pairs_sve)}/{M} ({100*len(pairs_sve)/M:.1f}%)")
    print(f"   阈值(threshold): {threshold:.3f}")
    print(f"   ✅ SVE 结果与 baseline (PyTorch) 结果一致")
...
def test_edge_cases() -> None:
    ...
    nz_counts, nz_col_indices, row_offsets = row_scan_sve(act, threshold, verbose=False)
...
def benchmark_performance(M: int, K: int, threshold: float, seed: int) -> None:
    ...
    # # 计算加速比
    # if lat_baseline > 0:
    #     speedup = lat_baseline / lat_sve
    #     print(f"   加速比: {speedup:.2f}x")
    #     if speedup > 1.0:
    #         print(f"   ✅ SVE 实现比 Baseline 快 {speedup:.2f}x")
    #     else:
    #         print(f"   ⚠️  SVE 实现比 Baseline 慢 {1.0/speedup:.2f}x")
    # else:
...
def benchmark_indices_generation_comparison(M: int, K: int, threshold: float, seed: int) -> None:
    ...
    # 获取实际非零元素数用于显示
    _, nz_col_indices_ref, _ = row_scan_sve(act, threshold, verbose=False)
    ...
    if lat_sve > 0:
        speedup_vs_baseline = lat_baseline / lat_sve
        print(f"   相对 SVE 的加速比: {speedup_vs_baseline:.2f}x" + 
              (f" (SVE 快 {speedup_vs_baseline:.2f}x)" if speedup_vs_baseline > 1.0 
               else f" (SVE 慢 {1.0/speedup_vs_baseline:.2f}x)"))
    ...
    if lat_sve > 0:
        speedup_vs_pytorch_style = lat_pytorch_style / lat_sve
        print(f"   相对 SVE 的加速比: {speedup_vs_pytorch_style:.2f}x" + 
              (f" (SVE 快 {speedup_vs_pytorch_style:.2f}x)" if speedup_vs_pytorch_style > 1.0 
               else f" (SVE 慢 {1.0/speedup_vs_pytorch_style:.2f}x)"))
    ...
    # 比较结果（注意 nz_col_indices 的类型可能不同：SVE 返回 uint32，baseline 返回 int64）
    nz_col_indices_sve_int64 = nz_col_indices_sve.to(dtype=torch.int64)
    nz_col_indices_pytorch_int64 = nz_col_indices_pytorch.to(dtype=torch.int64)

    match_sve_baseline = (
        torch.equal(nz_counts_sve, nz_counts_baseline) and
        torch.equal(nz_col_indices_sve_int64, nz_col_indices_baseline) and
        torch.equal(row_offsets_sve, row_offsets_baseline)
    )
    match_sve_pytorch = (
        torch.equal(nz_counts_sve, nz_counts_pytorch) and
        torch.equal(nz_col_indices_sve_int64, nz_col_indices_pytorch_int64) and
        torch.equal(row_offsets_sve, row_offsets_pytorch)
    )

    if match_sve_baseline:
        print(f"   ✅ SVE 与 Baseline 结果一致")
    else:
        print(f"   ❌ SVE 与 Baseline 结果不一致")

    if match_sve_pytorch:
        print(f"   ✅ SVE 与 PyTorch 风格实现结果一致")
    else:
        print(f"   ❌ SVE 与 PyTorch 风格实现结果不一致")

    # 性能总结
    print(f"\n📊 性能总结:")
    latencies = [
        ("SVE row_scan", lat_sve),
