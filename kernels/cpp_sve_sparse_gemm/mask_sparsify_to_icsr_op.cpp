// mask_sparsify_to_icsr_op.cpp
// Mask-based ICSR sparsify (indices only), scalar/OpenMP.
...
#pragma omp parallel for schedule(static)
  ...
  for (int64_t m = 0; m < M; ++m) {
    ...
    // For uint8_t mask, non-zero means valid.
#ifdef _OPENMP
#pragma omp simd reduction(+:nnz)
  ...
  // ---------------- Fused scan: row_offsets + keep_prefix ----------------
  ...
  // ---------------- Pass2: fill nz_col_indices + nz_counts ----------------
  // Each row writes [row_offsets[m], row_offsets[m+1]); each non-empty row writes its nz_counts slot; no atomics.
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
  for (int64_t m = 0; m < M; ++m) {
    ...
    // Small staging buffer (stack); batch stores to reduce traffic and branch pressure.
    const int BUFSZ = 64;
...
#ifndef NDEBUG
    // Debug: ensure written count matches row_nnz.
