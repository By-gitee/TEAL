...
torch::Tensor sparse_gemm_csc(
    ...
    TORCH_CHECK(nnz == row_indices.size(0), 
                "col_ptr[K] must equal row_indices length");
    TORCH_CHECK(nnz == values.size(0), 
                "values length must equal col_ptr[K] (nnz)");
    ...
    constexpr int64_t N_TILE = N/16;  // 可调：越大线程数越少，越小调度开销越大
